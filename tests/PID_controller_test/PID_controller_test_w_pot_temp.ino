...
//  gyro_dot[yaw] = (gyro_dot[yaw] * 0.7) + ((((float)gyro_raw[z]) / GYRO_SENSITIVITY) * 0.3);
  ...
  if(loop_iter > 0) {
    low_pass_filter(acc[roll], low_pass_roll_prior, low_pass_roll);
    low_pass_filter(acc[pitch], low_pass_pitch_prior, low_pass_pitch);
  } else {  // handle first iteration
    low_pass_roll = acc[roll];
    low_pass_pitch = acc[pitch];
  ...
  if(loop_iter > 1) {
    ...
    median_roll = acc[roll];
    median_pitch = acc[pitch];
  }

  acc[roll] = (LP_MED_COMBO_FILTER_ALPHA * low_pass_roll) + ((1 - LP_MED_COMBO_FILTER_ALPHA) * median_roll);
  acc[pitch] = (LP_MED_COMBO_FILTER_ALPHA * low_pass_pitch) + ((1 - LP_MED_COMBO_FILTER_ALPHA) * median_pitch);
  
  // compensate for offset IMU placement
  acc[roll]  -= 2.0; 
  acc[pitch] -= 3.4;  // previously 3 at steady state
  
  /* GYRO/ACC FUSION CALCULATION */
  if(startingGyroAnglesSet) {
    // use complementary filter to fuse sensor readings
    imu_deg[roll] = COMP_FILTER_ALPHA*(imu_deg[roll]+gyro[roll]) + (1-COMP_FILTER_ALPHA)*acc[roll]; 
  ...
  String angle_log = String(acc[pitch]);
  angle_log += "\t";
  angle_log += String(gyro[pitch]);
  angle_log += "\t";
  angle_log += String(imu_deg[pitch]);
  ...
  const float error_roll = imu_deg[roll] - rx_roll_deg;    // error = actual - desired measurement
...
void calculateSensorBias() {
  ...
  long gyro_raw[x] = 0, gyro_raw[y] = 0, gyro_raw[z] = 0;
  long acc_raw[x] = 0, acc_raw[y] = 0, acc_raw[z] = 0;
