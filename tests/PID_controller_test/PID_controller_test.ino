...
#ifdef DEBUG_ANGLES
  #define DEBUG_LOG_ANGLES(x) (Serial.println(x))
#else
  #define DEBUG_LOG_ANGLES(x)
#endif

#ifdef DEBUG_PID
  #define DEBUG_LOG_PID(x) (Serial.println(x))
#else
  #define DEBUG_LOG_PID(x)
#endif
...
static const float LP_FILTER_ALPHA   = 0.08;
static const float LP_MED_COMBO_FILTER_ALPHA = 0.9;
// PID controller variables
static const int PID_ACTIVATION_THROTTLE = MIN_RX_THROTTLE + 50;
static const float I_ERROR_THRESHOLD = 20.0; // error threshold that the integral PID term will be activated at
static const float MAX_PID = 400.0;      // maximum allowable PID output. Maximum allowable range 0<MAX_PID<500
// PID gains
static const float P_GAIN_ROLL = 2;  // 2.5
static const float D_GAIN_ROLL = 0; // 5; 10-20; 10 was pretty unpredictable...wobbly behavior
static const float I_GAIN_ROLL = 0;  // 0.03
...
static const float P_GAIN_YAW = 0;
...
static float acc_x_in_g, acc_y_in_g, acc_z_in_g;
static float acc_roll_radians, acc_pitch_radians;
...
static float low_pass_roll, low_pass_pitch;
static float low_pass_roll_prior, low_pass_pitch_prior;
static float median_roll, median_pitch;
// PID calculations
...
void loop() {
  ...
  gyro_roll_dot = ((float) gyro_raw_y) / GYRO_SENSITIVITY;
  gyro_pitch_dot = ((float) gyro_raw_x) / GYRO_SENSITIVITY;
  gyro_yaw_dot = ((float) gyro_raw_z) / GYRO_SENSITIVITY;
//  gyro_roll_dot = (gyro_roll_dot * 0.7) + ((((float)gyro_raw_y) / GYRO_SENSITIVITY) * 0.3);
//  gyro_pitch_dot = (gyro_pitch_dot * 0.7) + ((((float)gyro_raw_x) / GYRO_SENSITIVITY) * 0.3);
//  gyro_yaw_dot = (gyro_yaw_dot * 0.7) + ((((float)gyro_raw_z) / GYRO_SENSITIVITY) * 0.3);
  ...
  acc_x_in_g = ((float) acc_raw_x)/ACC_SENSITIVITY; // convert acc x from int to g
  acc_y_in_g = ((float) acc_raw_y)/ACC_SENSITIVITY; // convert acc y from int to g
  acc_z_in_g = ((float) acc_raw_z)/ACC_SENSITIVITY; // convert acc z from int to g
  acc_roll_radians = atan2(-acc_x_in_g, sqrt((acc_y_in_g*acc_y_in_g) + (acc_z_in_g*acc_z_in_g)));  // calculate roll in radians
  acc_pitch_radians  = atan2(acc_y_in_g, acc_z_in_g);  // calculate pitch in radians 
  ...
  enqueue(acc_roll_buffer, ACC_BUFF_SIZE, acc_roll);  
  enqueue(acc_pitch_buffer, ACC_BUFF_SIZE, acc_pitch);

  // apply low pass filter to accelerometer data
  if(loop_iter > 0) {
    low_pass_filter(acc_roll, low_pass_roll_prior, low_pass_roll);
    low_pass_filter(acc_pitch, low_pass_pitch_prior, low_pass_pitch);
  } else {  // handle first iteration
    low_pass_roll = acc_roll;
    low_pass_pitch = acc_pitch;
    low_pass_roll_prior = low_pass_roll;
    low_pass_pitch_prior = low_pass_pitch;
  }

  // apply median filter to accelerometer data
  if(loop_iter > 1) {
    median(acc_roll_buffer[0], acc_roll_buffer[1], acc_roll_buffer[2], median_roll);
    median(acc_pitch_buffer[0], acc_pitch_buffer[1], acc_pitch_buffer[2], median_pitch);
  } else {  // handle first two iterations
    median_roll = acc_roll;
    median_pitch = acc_pitch;
  }

  acc_roll = (LP_MED_COMBO_FILTER_ALPHA * low_pass_roll) + ((1 - LP_MED_COMBO_FILTER_ALPHA) * median_roll);
  acc_pitch = (LP_MED_COMBO_FILTER_ALPHA * low_pass_pitch) + ((1 - LP_MED_COMBO_FILTER_ALPHA) * median_pitch);
  
  // compensate for offset IMU placement
  acc_roll  -= 2.0; 
  acc_pitch -= 3.4;  // previously 3 at steady state
  
  /* GYRO/ACC FUSION CALCULATION */
  if(startingGyroAnglesSet) {
    // use complementary filter to fuse sensor readings
    imu_roll_deg = COMP_FILTER_ALPHA*(imu_roll_deg+gyro_roll) + (1-COMP_FILTER_ALPHA)*acc_roll; 
    imu_pitch_deg = COMP_FILTER_ALPHA*(imu_pitch_deg+(gyro_pitch_dot*delta_t)) + (1-COMP_FILTER_ALPHA)*acc_pitch;   
  } else {
    // accomodate for any starting angular offset due to unlevel surface
    gyro_roll = acc_roll;
    gyro_pitch = acc_pitch;
    imu_roll_deg = gyro_roll;
    imu_pitch_deg = gyro_pitch;
    startingGyroAnglesSet = true;
  }

  String angle_log = String(acc_pitch);
  angle_log += "\t";
  angle_log += String(gyro_pitch);
  angle_log += "\t";
  angle_log += String(imu_pitch_deg);
  DEBUG_LOG_ANGLES(angle_log);

  // limit RX inputs
  if(rx_throttle_pulse > MAX_RX_THROTTLE) { rx_throttle_pulse = MAX_RX_THROTTLE; } // maximum throttle === MAX_RX_THROTTLE (~1800us)
  if(rx_roll_pulse < MIN_RX_ANGLE_PULSE) { rx_roll_pulse = MIN_RX_ANGLE_PULSE; }
  if(rx_roll_pulse > MAX_RX_ANGLE_PULSE) { rx_roll_pulse = MAX_RX_ANGLE_PULSE; }
  if(rx_pitch_pulse < MIN_RX_ANGLE_PULSE) { rx_pitch_pulse = MIN_RX_ANGLE_PULSE; }
  if(rx_pitch_pulse > MAX_RX_ANGLE_PULSE) { rx_pitch_pulse = MAX_RX_ANGLE_PULSE; }

  /* PID CALCULATIONS - ROLL */
  // convert roll range from [MIN_RX_ANGLE_PULSE, MAX_RX_ANGLE_PULSE] to [-MAX_ANGLE_DEG, +MAX_ANGLE_DEG]
  float rx_roll_deg = ((float)((rx_roll_pulse-MIN_RX_ANGLE_PULSE)*2*MAX_ANGLE_DEG))/(MAX_RX_ANGLE_PULSE-MIN_RX_ANGLE_PULSE) - MAX_ANGLE_DEG; 
  if(rx_roll_pulse >= 1492 && rx_roll_pulse <= 1508) { rx_roll_deg = 0.0; }   // set degree to 0.0 if pulse is around 1500
  
  const float error_roll = imu_roll_deg - rx_roll_deg;    // error = actual - desired measurement
  const float P_roll = P_GAIN_ROLL * error_roll;    // determine P term
  const float D_roll = D_GAIN_ROLL * (error_roll - error_prior_roll); // determine D term
  I_roll += I_GAIN_ROLL * error_roll;   // determine I term

  // prevent integral windup when large/prolonged error occurs
  if(abs(error_roll) > I_ERROR_THRESHOLD) { 
    I_roll = 0.0;
  }
  
  float roll_pid = P_roll + I_roll + D_roll;
  error_prior_roll = error_roll;  // track error for next loop iteration

  if(roll_pid > MAX_PID) { roll_pid = MAX_PID; }    // constrain pid to maximum value
  if(roll_pid < -MAX_PID) { roll_pid = -MAX_PID; }  // constrain pid to minimum value

  //pid_log = roll_pid + "\t" + pitch_pid + "\t" + yaw_pid;
  String pid_log = String(rx_roll_deg);
  pid_log += "\t";
  pid_log += imu_roll_deg;
  pid_log += "\t";
  pid_log += roll_pid;
  DEBUG_LOG_PID(pid_log);
  ...
  if(rx_throttle_pulse > PID_ACTIVATION_THROTTLE) {
    esc_1_pulse = rx_throttle_pulse + roll_pid; // + roll, - pitch
    esc_2_pulse = rx_throttle_pulse + roll_pid; // + roll, + pitch
    esc_3_pulse = rx_throttle_pulse - roll_pid; // - roll, + pitch
    esc_4_pulse = rx_throttle_pulse - roll_pid; // - roll, - pitch
  } else {
...
void low_pass_filter(float input, float& low_pass_prior, float& output) {
  output = (LP_FILTER_ALPHA * input) + ((1-LP_FILTER_ALPHA) * low_pass_prior);
  low_pass_prior = output;
}
